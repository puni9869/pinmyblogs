...
func TestConfigProviderStruct(t *testing.T) {
	...
	want := "config.ConfigProvider{EnableSSL:false, ShareDataOverMail:false, Database:map[string]config.DatabaseObj(nil), AppConfig:config.AppConfig{Debug:false, SecretKey:\"\", DefaultPort:\"\", CustomPort:\"\", UseCDN:false, Hostname:\"\", DataBaseType:\"\"}, Authentication:config.Authentication{EnableLogin:false, OpenDisabledAccountByEmailLink:false, EnableRegistration:false, EnableForgotPassword:false}, Mailer:config.Mailer{SmtpHost:\"\", SmtpPort:0, EmailId:\"\", BccEmailId:\"\", Username:\"\", Password:\"\"}}"
	got := fmt.Sprintf("%#v", c)
	fmt.Println(got)
